---
titel: Goggins Battle Plan – Rauchfrei
datum: 2026-05-08
aktualisiert: 2026-05-09
tags: [rauchfrei, mindset, aar, visualization]
quelle: "[[Cant Hurt Me - David Goggins]]"
verbunden: ["[[Trigger-Analyse]]", "[[Cookie Jar - Meine Siege]]"]
status: aktiv
modus: C
startdatum: 2026-06-01
---

# Goggins Battle Plan – Rauchfrei

> [!quote]
> "Don't stop when you are tired. Stop when you are done." – Goggins

Du hattest 27.04.2026 als Startdatum geplant – und nicht gestartet. Das ist **kein Versagen**. Das ist **Daten**. Goggins macht nach jedem Misserfolg eine AAR (After Action Review) und plant sofort den nächsten Versuch. Das hier ist deine.

---

## Teil 1: AAR – Warum 27.04. nicht geklappt hat

> AAR durchgeführt 09.05.2026 mit /goggins. Brutale Ehrlichkeit. Keine Ausreden.

### Was war der Plan?

Am Mo 27.04.2026 mit Pflaster Stufe 2 + Kaugummi + Smoke Free App + Trigger-Strategie aufhören. Im Vault dokumentiert seit ~13.04. (Jahresplan-Loop geschlossen).

### Was lief gut in der Vorbereitung?

- Trigger-Analyse erstellt ✓ ([[Trigger-Analyse]])
- Ersatz-Handlungen pro Trigger definiert ✓
- Datum im Jahresplan + Cookie Jar dokumentiert ✓
- → **Vault-Vorbereitung war komplett. Real-Life-Vorbereitung war null.**

### Was lief nicht gut?

**Wörtliche Antwort vom 09.05.2026:**
> „Mir hat die Motivation dazu gefehlt, es einfach aufzuhören statt einfach den Drang dazu weiterzumachen. Ich habe mir weder Pflaster noch Kaugummi gekauft. Die App habe ich auch nicht gemacht. Es lief leider weiter wie sonst. Ich hab gar nicht daran gedacht, dass ich aufhören wollte. Ich hab mit niemandem geredet."

Konkret:
- [x] Pflaster nicht gekauft
- [x] Kaugummi nicht gekauft
- [x] Smoke Free App nicht installiert
- [x] Verlobte nicht informiert
- [x] Visualization nicht gemacht
- [x] Am 27.04. nicht mal gedanklich präsent gehabt, dass aufhören geplant war
- [x] Trigger-Tabelle nicht reviewt

### Was war mein Mindset?

**Wörtliche Antwort vom 09.05.2026 (auf die Frage warum keine Schritte):**
> „(a) Ich wollte gar nicht wirklich aufhören – ich wollte nur den Gedanken haben, dass ich aufhöre."

→ **Diagnose: Wollen-Problem, nicht Willenskraft-Problem.** Solange das Wollen nicht da ist, scheitert jeder Plan, egal wie gut er im Vault aussieht.

### Was muss konkret anders sein?

Goggins-Reframe statt „mehr Motivation":
- **Motivation streichen.** Sie kommt und sie geht. Wer auf sie wartet, hat schon verloren.
- **Externe Anker statt innerer Wille.** Nicht „ich will" sondern „ich kann nicht mehr nachträglich kippen".
- **Modus C gewählt:** Force the want through the action. Aufhören OHNE dass das Bauchgefühl zustimmt – aber NUR mit allen 4 Ankern stehend.

### Lehre für 01.06.

> Der Plan war im Vault perfekt. Im Leben null. Diesmal: Plan ist erst real wenn die ersten Anker außerhalb des Vaults stehen (Verlobte gesprochen, Pflaster im Schrank, App auf Handy).

---

## Teil 2: Armored Mind – Visualization für nächsten Start

> Goggins: Vor jeder schweren Mission spielst du die Hölle mental durch. Dann ist sie real keine Überraschung mehr.

### Schritt 1: Stelle dir VORHER vor – die ersten 72 Stunden

**Tag 1 morgens:**
- Du wachst auf, willst nach Kaffee greifen → **Kaugummi liegt SCHON BEREIT** auf der Kaffeemaschine
- Erste Stunde: Verlangen kommt in Wellen. **Es ist normal. Es geht vorbei.**
- Visualisiere: Du schaffst diesen ersten Tag.

**Tag 1 nach dem Mittagessen (dein 10er Trigger):**
- Du stehst auf, **gehst BEWUSST raus für 5 Min Spaziergang**
- Nicht zur Raucherecke. Andere Route.
- Du fühlst das Verlangen, lässt es kommen, lässt es gehen.

**Tag 2 – die Falle:**
- Du fühlst dich gut. "Vielleicht eine reicht..."
- DAS IST DER MOMENT. Goggins: "It's a one-second decision."
- Greif in den [[Cookie Jar - Meine Siege]] – fühl einen Sieg
- Sage laut: "Ich bin Nicht-Raucher."

**Tag 3 – körperliches Tief:**
- Stimmung kippt, Reizbarkeit, Kopfschmerzen
- Goggins: "Pain is data, not stop sign."
- 40%-Regel: Dein Verstand will aufgeben → du hast noch 60% Reserve

### Schritt 2: Die Trigger einzeln durchgehen (ergänzt zu deiner Tabelle)

| Trigger | Goggins-Antwort |
|---------|-----------------|
| Morgens Kaffee | **Vor dem Aufstehen visualisieren**: ich greife zum Kaugummi, nicht zur Zigarette. |
| Nach dem Essen | **Sofort aufstehen**, anderer Raum, Wasser trinken. Nicht überlegen. |
| Stress | **3 tiefe Atemzüge = mein Soul-Take-Moment.** Ich nehme dem Stress die Macht über mich. |
| Arbeitspause | **Pause nehmen, aber andere Aktivität.** Treppe hoch und runter. |
| Bier/Ausgehen | **Erste 2 Wochen: vermeiden.** Goggins: "Avoid the fight you don't need yet." |

### Schritt 3: Tägliches Skript

**Morgens (Spiegel-Check):**
> "Ich bin Nicht-Raucher. Heute ist Tag X. Ich habe schon Härteres durchgezogen."

**Bei Verlangen:**
> "Das ist 90 Sekunden. Ich bin stärker als 90 Sekunden."

**Abends:**
> "Ich habe heute den Tag durchgezogen. Cookie ins Jar."

---

## Teil 3: Neues Startdatum – LOCKED IN

> [!important] Tag 1: **Mo 01.06.2026**
> **Modus:** C – Force the want through the action
> **Begründung:** Fällt mit Zyklus-2-Start im Gym zusammen → Identitäts-Reset auf 2 Ebenen gleichzeitig. BU-Frist April 2027 noch im sauberen Puffer (10 Mo rauchfrei bis dahin).

### Die 4 nicht-verhandelbaren Anker

Fehlt einer → C automatisch abgesagt.

| # | Anker | Frist |
|---|-------|-------|
| 1 | Verlobte informiert + ihre Rolle definiert | So 10.05. |
| 2 | Pflaster Stufe 2 (14 mg) + Nicotinell Kaugummi gekauft | Mi 13.05. |
| 3 | Smoke Free App installiert + 01.06. eingetragen | So 17.05. |
| 4 | Geldbinde-Regel mit Verlobter besprochen (5 €/Zigarette in Hochzeitsfonds) | So 17.05. |

### Vor-Start Plan (vorletzte Woche bis 31.05.)

Aktionen → **Google Tasks** (Liste „Meine Aufgaben"):
- Mo 18.05. – Visualization-Skript starten (5 Min/Abend)
- So 24.05. – Trigger-Tabelle reviewen
- Sa 30.05. – Cookie Jar lesen + neue Siege ergänzen
- So 31.05. – **Letzter Rauchtag** (bewusst, nicht zufällig)
- So 31.05. abends – alle Zigaretten/Anzünder weg, Pflaster bereit fürs Gesicht des Weckers

---

## Teil 4: Verbindung zu größeren Zielen

**Warum das größer ist als nur Rauchfrei:**

1. **Hochzeit 02.10.2027** – rauchfrei zur eigenen Hochzeit
2. **Verlobung 19.09.2026** – starkes Bild für sie
3. **[[Fett to Fit]]** – Lungenkapazität fürs Training
4. **Finanzen** – ~150-200 €/Monat extra für [[Hochzeit]]-Sparen
5. **Mentale Identität** – "Ich bin jemand, der schwere Dinge durchzieht"

> Goggins: "Wenn du eine harte Sache durchziehst, callouses du den Verstand für ALLE harten Sachen."

---

## Teil 5: Wenn du wieder rauchst (Realismus)

> Goggins ist NICHT romantisch. Er weiß: Manche Schlachten gehen verloren.

**Das ist KEINE Erlaubnis – das ist Vorbereitung.**

Wenn du rauchst:
1. **Sofortige AAR:** Was war der Auslöser, der gewonnen hat?
2. **Kein Spiral-Down:** Eine Zigarette ≠ Kapitulation
3. **Cookie Jar:** Du hast schon X Tage geschafft – das gilt
4. **Nächster Versuch:** Innerhalb von 7 Tagen, neuer Tag 1
5. **Kein Schamgefühl:** Goggins: "Failure is data."

---

## Wochenrythmus während Rauchfrei-Phase

**Sonntag (Weekly Review):**
- AAR der Woche
- Cookie Jar-Update
- Trigger-Notes aktualisieren

**Täglich:**
- Spiegel-Check morgens
- Bei Verlangen: 90-Sek-Regel + Cookie Jar
- Abends: 1 Eintrag in Daily Note

**Bei Trigger-Sieg:**
- Sofort Cookie Jar erweitern
- "Ich habe X überstanden" ins Daily Note

---

## Verwandte Notizen

- [[Cant Hurt Me - David Goggins]] – die Methodik
- [[Trigger-Analyse]] – deine Trigger-Tabelle
- [[Cookie Jar - Meine Siege]] – mentale Munition
- [[Fortschritt]] – Tag-für-Tag Tracking
- [[Rauchfrei]] – Projekt-Hub

---

> [!quote] Letzter Goggins-Satz für dich
> "Most people who are merely inspired or motivated will quit. The few who remain encounter even more pain. Welcome to the few."
